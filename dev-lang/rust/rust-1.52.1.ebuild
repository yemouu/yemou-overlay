...
pkg_setup() {
	...
	LIBGIT2_NO_PKG_CONFIG=1 #749381
	# needed to build gentoo's musl target and normal musl target
...
src_configure() {
	use elibc_musl || local rust_target="" rust_targets="" arch_cflags
	use elibc_musl && local rust_target="" rust_targets="\"$CHOST\"" arch_cflags

	# Collect rust target names to compile standard libs for all ABIs.
	for v in $(multilib_get_enabled_abi_pairs); do
		use elibc_musl || rust_targets="${rust_targets},\"$(rust_abi $(get_abi_CHOST ${v##*.}))\""
		use elibc_musl && rust_targets="${rust_targets},\"$(rust_abi $(get_abi_CHOST ${v##*.}) | sed s/gnu/musl/)\""
	done
	if use wasm; then
	...
	use elibc_musl || rust_target="$(rust_abi)"
	use elibc_musl && rust_target="$CHOST"
